...
compiler_path = os.path.join(extdir,'closure-compiler.jar')
...
def checkDependencies():
    ...
    if not (os.path.exists(closure_dir) and  os.path.exists(closure_deps_file)):
        ...
        retcode = subprocess.Popen(subprocess.list2cmdline(["git","checkout","161037749f1efee3142630bba5092709cb09f796"]),shell=True,cwd=closure_dir).wait()
    ...
    if not os.path.exists(compiler_path):
        ...
        urlretrieve("https://dl.google.com/closure-compiler/compiler-20161024.zip",zip_path,rephook)
        print ('\nUnzipping...')
        zippedFile = zipfile.ZipFile(zip_path)
        zippedFile.extract('closure-compiler-v20161024.jar',extdir)
        zippedFile.close()
        print ('Cleanup')
        os.unlink(zip_path)
        os.rename(os.path.join(extdir,'closure-compiler-v20161024.jar'), compiler_path)
